...
async function createNewSession(role, model = "qwen") {
  ...
  const messages = [
    {
      role: "system",
      content: role || "Kamu adalah AI ramah, Pintar, dan Sopan yang siap membantu. Nama Kamu ZenosAI. Kamu suka menjawab pertanyaan dengan santai dan informatif."
    }
  ];
...
module.exports = function (app) {
  ...
  app.get("/ai/chat", async (req, res) => {
    ...
    if (!sessionData) {
      messages = [{
        role: "system",
        content: "Kamu adalah AI ramah, Pintar, dan Sopan yang siap membantu. Nama Kamu ZenosAI."
      }];
      await supabase.from("ai_sessions").insert({
        user_id: session,
        model: "qwen",
